...
function calculatePortfolioMetrics(holdings) {
  ...
  try {
    ...
    holdings.forEach(holding => {
      ...
      const costBasis = holding.costInEUR || (holding.costPerShare * holding.shares);
...
export async function aggregatePortfolioData(portfolio, currentPrices = {}) {
  ...
  try {
    // Enhance holdings with current prices
    const enhancedHoldings = portfolio.holdings.map(holding => ({
      ...holding,
      currentPrice: currentPrices[holding.symbol] || null,
    }));

    // Calculate portfolio metrics
    const metrics = calculatePortfolioMetrics(enhancedHoldings);
    ...
    const portfolioData = {
      ...
      holdings: metrics?.holdings || enhancedHoldings,
      metrics,
      ...
      summary: {
        ...
        diversificationScore: metrics?.diversification?.score || 0,
        top5Concentration: metrics?.concentration?.top5 || 0,
      },
      
      aggregatedAt: new Date().toISOString(),
    };

    console.log(`[AIDataAggregator] Successfully aggregated portfolio data`);
