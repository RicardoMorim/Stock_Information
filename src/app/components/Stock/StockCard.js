    ...
    <div className="bg-gray-800 shadow-lg rounded-lg p-4 md:p-6 hover:shadow-xl transition-shadow duration-200 ease-in-out flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-2">
          <h3
            className="text-xl md:text-2xl font-semibold text-blue-400 truncate"
            title={name || symbol}
          >
            {name || symbol}
          </h3>
          {exchangeShortName && exchangeShortName !== "N/A" && (
            <span className="text-xs bg-gray-700 text-gray-300 px-2 py-1 rounded-full uppercase tracking-wider">
              {exchangeShortName}
            </span>
          )}
        </div>
        <p className="text-sm text-gray-400 mb-1 truncate" title={symbol}>
          Symbol: {symbol}
        </p>
      ...
      <div className="mt-auto">
        {hasPriceData && (
          <div className="flex justify-between items-center mb-3">
            ...
            {hasChangeData && (
              <span
                className={`text-sm md:text-base font-medium px-2 py-1 rounded-md ${
                  isPositiveChange
        ...
        {!hasPriceData && <div className="h-10 mb-3"></div>}
