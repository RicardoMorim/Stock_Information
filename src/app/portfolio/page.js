...
export default function PortfolioPage() {
    ...
    const portfolioSummary = useMemo(() => {
        if (!portfolio?.summary) {
            console.log("[PortfolioPage] portfolioSummary: portfolio or portfolio.summary is undefined, returning defaults."); // Page Log 6
            return {
        ...
        console.log("[PortfolioPage] portfolioSummary: calculating with portfolio.summary:", portfolio.summary); // Page Log 7
        return {
    ...
    if (isLoading && !portfolio?.data) {
        console.log("[PortfolioPage] Rendering PortfolioSkeleton (isLoading true and no portfolio data)."); // Page Log 11
        return <PortfolioSkeleton />;
    }

    if (error && !portfolio?.data) {
        console.log("[PortfolioPage] Rendering error display (error present and no portfolio data)."); // Page Log 12
        return (
            <div className="container mx-auto p-6 bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center">
                ...
                <button
                    onClick={() => { setIsLoading(true); fetchPortfolio(); }}
